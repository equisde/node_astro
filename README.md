# A Node.js Astro Client!

![Astro](https://raw.githubusercontent.com/equisde/node-astro/master/node-astro.png)

node-astro is a Astro client for Node.js. It is a fork of the excellent Kapitalize Bitcoin Client (now removed from GitHub) intended for use with Astro. The purpose of this repository is:

* Provide a one-stop resource for the Node.js developer to get started with Astro integration.
* Prevent would-be Astro web developers worrying whether a Bitcoin client will work out of the box.
* Promote Node.js development of Astro web apps.
* Identify and address any incompatibilities with the Astro and Bitcoin APIs that exist now and/or in the future.

## Dependencies

You'll need a running instance of [astrod](https://github.com/equisde/astro) to connect with.

Then, install the node-astro NPM package.

`npm install node-astro`
...
```js
var astro = require('node-astro')()

astro.auth('myusername', 'mypassword')

astro.getDifficulty(function() {
...
```js
astro.getNewAddress(function(err, address) {
...
```js
astro.exec('getNewAddress')

astro.exec('getbalance', function(err, balance) {
...
<td> Add a nrequired-to-sign multisignature address to the wallet. Each key is a astro address or hex-encoded public key. If [account] is specified, assign address to [account]. </td>
<td> N
</td></tr>
<tr>
<td> backupwallet </td>
...
<td> dumpprivkey </td>
<td> [astroaddress] </td>
<td> Reveals the private key corresponding to <astroaddress< </td>
<td> Y
</td></tr>
<tr>
<td> encryptwallet </td>
...
<td> [account] </td>
<td> Returns the current astro address for receiving payments to this account. </td>
<td> N
</td></tr>
<tr>
...
<td> getgenerate </td>
<td> </td>
<td> Returns true or false whether astrod is currently generating hashes </td>
<td> N
</td></tr>
<tr>
...
<td> getnewaddress </td>
<td> [account] </td>
<td> Returns a new astro address for receiving payments.  If [account] is specified (recommended), it is added to the address book so payments received with the address will be credited to [account]. </td>
<td> N
</td></tr>
<tr>
...
<td> [astroaddress] [minconf=1] </td>
<td> Returns the total amount received by <astroaddress< in transactions with at least [minconf] confirmations. While some might consider this obvious, value reported by this only considers *receiving* transactions. It does not check payments that have been made *from* this address. In other words, this is not "getaddressbalance". Works only for addresses in the local wallet, external addresses will always show 0. </td>
<td> N
</td></tr>
<tr>
...
<p>To get a list of accounts on the system, execute astrod listreceivedbyaddress 0 true
</p>
</td>
<td> N
</td></tr>
...
<tr>
<td> sendfrom </td>
<td> [fromaccount] [toastroaddress] [amount] [minconf=1] [comment] [comment-to] </td>
...
<td> stop </td>
<td> </td>
<td> Stop astro server. </td>
<td> N
</td></tr>
<tr>
<td> validateaddress </td>
<td> [astroaddress] </td>
<td> Return information about [astroaddress]. </td>
<td> N
</td></tr>
<tr>
<td> verifymessage </td>
<td> [astroaddress] [signature] [message] </td>
...
You may pass an optional function `passphrasecallback` to the `node-astro` initialization function to manage wallet unlocks. `passphrasecallback` should be a function accepting three arguments:
...
By default `astrod` exposes its JSON-RPC interface via HTTP; that is, all RPC commands are transmitted in plain text across the network! To secure the JSON-RPC channel you can supply `astrod` with a self-signed SSL certificate and an associated private key to enable HTTPS. For example, in your `astro.conf`:

    rpcssl=1
    rpcsslcertificatechainfile=/etc/ssl/certs/astrod.crt
    rpcsslprivatekeyfile=/etc/ssl/private/astrod.pem

In order to securely access an SSL encrypted JSON-RPC interface you need a copy of the self-signed certificate from the server: in this case `astrod.crt`. Pass your self-signed certificate in the `ca` option and set `https: true` and node-astro is secured!
    
```js
var fs = require('fs')

var ca = fs.readFileSync('astrod.crt')
...
nodeunit test/test-node-astro.js
```

## Donations

[Astro](http://www.astro) donation address is Acz8nqhcfAUAdWPrKefyNoWpSLmSF3ZoHr
